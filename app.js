...
function handleRenewalCertificate(files) {
  ...
  // Destructuring files array takes the first file in array
  [renewalFile] = files;

  // validate file type (pdf)
  if (renewalFile.type !== "application/pdf") {
    showValidationDialog(
      "Document must be public document format (PDF).",
      "uploadArea"
    );
  ...
  $('#renewal-file-info').html(`<h3>${renewalFile.name}</h3>`).show();
...
function submitRenewal() {
  ...
  if (!renewalFile) {
    showValidationDialog(
      "Please upload the renewal certificate PDF.",
      "renewalUploadArea"
    );
    isValid = false;
    return;
...
function uploadRenewalDocument(digest) {
  ...
  reader.readAsArrayBuffer(renewalFile);
